...
export default function MembershipHero({ hero }) {
  ...
  const rawStatus = hero?.status || "pending";
  const status = `${rawStatus[0].toUpperCase()}${rawStatus.slice(1)} Member`;
      ...
      <div className="flex items-center gap-6 relative z-10">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1.5">
            ...
            <span className={`${getStatusBadgeClass(rawStatus)} text-[11px] font-semibold px-3 py-1 rounded-full`}>
              {status}
            </span>
          </div>
